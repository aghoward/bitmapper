...
class BitmapFactoryTests : public ::testing::Test {
    protected:
        ...
        MockBitmapHeaderFactory _mockBitmapHeaderFactory;
        MockBitmapCoreHeaderFactory _mockBitmapCoreHeaderFactory;
        MockColorTableHeaderFactory _mockColorTableHeaderFactory;
...
TEST_F(BitmapFactoryTests, Create_CallsColorTableHeaderFactoryWithEvenColors) {
    ...
    EXPECT_CALL(_mockBitmapHeaderFactory, Create(_, _))
        .Times(Exactly(2));
    EXPECT_CALL(_mockBitmapCoreHeaderFactory, Create(_, _))
        .Times(Exactly(1));
...
TEST_F(BitmapFactoryTests, Create_CallsBitmapCoreHeaderFactoryWithFilledPixelArray) {
    ...
    EXPECT_CALL(_mockBitmapCoreHeaderFactory, Create(expected, bitsPerPixel))
        .Times(Exactly(1));
    EXPECT_CALL(_mockColorTableHeaderFactory, Create(_, _, _))
        .Times(Exactly(1));
